...
class EmailLogViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            print(f"[email] Sent to {email.recipient_email}", flush=True)
            email.status = 'Sent'
        except Exception as e:
            print(f"[email] FAILED to {email.recipient_email}: {e}", flush=True)
    ...
    def batch_send(self, request):
        ...
        try:
            connection.open()
            print(f"[email] SMTP connection opened successfully for batch.", flush=True)
        except Exception as e:
        ...
        for emp_id in employee_ids:
            ...
            try:
                ...
                success_count += 1
                print(f"[email] Batch sent to {emp.email}", flush=True)
            except Exception as e:
                logger.exception(f"Failed to send email to {emp.email}: {e}")
                print(f"[email] Batch FAILED to {emp.email}: {e}", flush=True)
